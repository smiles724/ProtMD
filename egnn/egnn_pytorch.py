...
class EGNN_Network(nn.Module):
    def __init__(self, *, depth, dim, num_tokens=None, num_edge_tokens=None, num_positions=None, edge_dim=0,
                 ...
                 global_linear_attn_dim_head=64, num_global_tokens=4, num_prompt=1, aggregate=True, **kwargs):
        super().__init__()
        assert not (exists(num_adj_degrees) and num_adj_degrees < 1), 'make sure adjacent degrees is greater than 1'
        self.aggregate = aggregate
        if self.aggregate:
            self.out = predictor(dim)
        self.prompt_embed = nn.Embedding(num_prompt, dim)if num_prompt > 1 else None
    ...
    def forward(self, feats, coors, prompt=None, adj_mat=None, edges=None, mask=None, return_coor_changes=False):
        ...
        if exists(self.prompt_embed):
            feats += self.prompt_embed(prompt)
        ...
        if self.aggregate:
            # 输出graph-level的特征，虽然需要考虑mask，但不考虑mask效果更好……
            return coors, self.out(feats)
        else:
            # 输出维度为(B,N,3), (B,N,dim)
            return coors, feats
